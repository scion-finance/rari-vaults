...
contract VaultUpgradable is Initializable, ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function setUnderlyingIsWETH(bool newUnderlyingIsWETH) external onlyOwner {
        ...
        require(!newUnderlyingIsWETH || ERC20(address(UNDERLYING)).decimals() == 18, "WRONG_DECIMALS");
    ...
    function withdrawFromStrategy(Strategy strategy, uint256 underlyingAmount) public requiresAuth {
        ...
        if (strategy.isCEther()) IWETH(payable(address(UNDERLYING))).deposit{value: underlyingAmount}();
    }
    ...
    function migrateStrategy(
        ...
        if (queueIndex < withdrawalQueue.length) replaceWithdrawalQueueIndex(queueIndex, newStrategy);
    ...
    function pullFromWithdrawalQueue(uint256 underlyingAmount, uint256 float) internal {
        ...
        for (; ; currentIndex--) {
            ...
            unchecked {
                ...
                uint256 strategyBalanceAfterWithdrawal = strategyBalance > withdrawn ? strategyBalance - withdrawn : 0;
    ...
    /// @dev Strategies that are untrusted, duplicated, or have no balance are
    /// filtered out when encountered at withdrawal time, not validated upfront.
    function replaceWithdrawalQueueIndex(uint256 index, Strategy replacementStrategy) public requiresAuth {
    ...
    function seizeStrategy(Strategy strategy, IERC20[] calldata tokens) external requiresAuth {
        ...
        if (strategyBalance > 0) {
            try Strategy(strategy).redeemUnderlying(type(uint256).max) {} catch Error(string memory err) {
                // redeem may fail because of price mismatch - we want to revert seize in this case
                require(
                    keccak256(abi.encodePacked((err))) != keccak256(abi.encodePacked(("HLP: PRICE_MISMATCH"))),
                    err
                );
            }
        }
